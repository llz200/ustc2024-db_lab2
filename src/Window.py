...
class add_timescoreWindow(QDialog):
    ...
    def get_text(self):
        ...
        if acceptflag == 2:
            if self.kind == 0:
                info = selects.select_prize_name(self.db, self.cursor, self.name)
                if (info == None):
                    widget = QWidget()
                    QMessageBox.information(widget, '信息', '奖项不存在') #触发的事件时弹出会话框
                    return
                info = selects.select_student_prize_one(self.db, self.cursor, self.ID, self.name)
            if self.kind == 1:
                info = selects.select_punish_name(self.db, self.cursor, self.name)
                if (info == None):
                    widget = QWidget()
                    QMessageBox.information(widget, '信息', '惩罚不存在') #触发的事件时弹出会话框
                    return
                info = selects.select_student_punish_one(self.db, self.cursor, self.ID, self.name)
            if self.kind == 2:
                info = selects.select_class_ID(self.db, self.cursor, self.name)
                if (info == None):
                    widget = QWidget()
                    QMessageBox.information(widget, '信息', '课程不存在') #触发的事件时弹出会话框
                    return
                info = selects.select_student_class_score(self.db, self.cursor, self.ID, self.name)
            if (info != None):
                widget = QWidget()
...
class New_ClassWindow(QDialog):
    ...
    def get_text(self):
        ...
        if (len(self.input_box1.text()) != 0):
            acceptflag += 1
            self.ID = self.input_box1.text()
            info = selects.select_class_ID(self.db, self.cursor, self.ID)
            if (info != None):
                widget = QWidget()
...
class Main_Window(QWidget):
    def __init__(self, db, cursor):
        ...
        self.btn4 = QPushButton('按学号查询具体信息', self)
    ...
    def add_class(self):
        ...
        if self.newWindow.exec():
            ...
            classes.add_class(self.db, self.cursor, newID, newName, newPoint)
            widget = QWidget()
            QMessageBox.information(widget, '信息', '添加成功') #触发的事件时弹出会话框
    ...
    def add_prize(self):
        ...
        if self.newWindow.exec():
            ...
            prizes.add_prize(self.db, self.cursor, newName, newGrade)
            widget = QWidget()
            QMessageBox.information(widget, '信息', '添加成功') #触发的事件时弹出会话框
        ...
        if getID.exec():
            ...
            if oldinfo == None:
                ...
                if self.changeWindow.exec():
                    ...
                    classes.change_class(self.db, self.cursor, oldID, 2, newPoint)
    ...
    def delete_class(self):
        ...
        if getID.exec():
            ...
            if oldinfo == None:
                ...
                classes.delete_class(self.db, self.cursor, oldID)
                widget = QWidget()
                QMessageBox.information(widget, '信息', '删除成功') #触发的事件时弹出会话框
    ...
    def delete_prize(self):
        ...
        if getID.exec():
            oldID = getID.get_entered_text()
            oldinfo = selects.select_prize_name(self.db, self.cursor, oldID)
            if oldinfo == None:
                widget = QWidget()
                QMessageBox.information(widget, '信息', '奖项不存在')
            else:
                prizes.delete_prize(self.db, self.cursor, oldID)
                widget = QWidget()
                QMessageBox.information(widget, '信息', '删除成功') #触发的事件时弹出会话框
    ...
    def delete_punish(self):
        ...
        if getID.exec():
            oldID = getID.get_entered_text()
            oldinfo = selects.select_punish_name(self.db, self.cursor, oldID)
            if oldinfo == None:
                widget = QWidget()
                QMessageBox.information(widget, '信息', '惩罚不存在')
            else:
                punishments.delete_punishment(self.db, self.cursor, oldID)
                widget = QWidget()
                QMessageBox.information(widget, '信息', '删除成功') #触发的事件时弹出会话框
    ...
    def add_punish_for_student(self):
        ...
        if self.newWindow.exec():
            ...
            punishments.add_punishtime(self.db, self.cursor, newID, newName, newTime)
            widget = QWidget()
            QMessageBox.information(widget, '信息', '添加成功') #触发的事件时弹出会话框
    ...
    def add_score(self):
        ...
        if self.newWindow.exec():
            ...
            students.add_score(self.db, self.cursor, newID, newName, newScore)
            widget = QWidget()
            QMessageBox.information(widget, '信息', '添加成功') #触发的事件时弹出会话框
    ...
    def delete_score(self):
        ...
        if self.newWindow.exec():
            ...
            oldinfo = selects.select_student_class_score(self.db, self.cursor, newsID, newcID)
            if oldinfo == None:
                widget = QWidget()
                QMessageBox.information(widget, '信息', '成绩不存在')
            else:
                students.delete_score(self.db, self.cursor, newsID, newcID)
                widget = QWidget()
                QMessageBox.information(widget, '信息', '删除成功') #触发的事件时弹出会话框
